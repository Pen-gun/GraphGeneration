export const makePrompt = (topic) => {
  return `
You are an assistant that outputs JSON only.

Task:
1) Provide 6 short, plain-language bullet points that explain "${topic}" so a beginner can memorize them easily.
2) Create a Mermaid diagram (graph TD) that shows main components and flow for "${topic}" so it helps with memory (nodes and arrows).
